...
class PackagePlatformT2(PackagePlatform):
    def ar_platform(self, vendor_obj_list, output_file):
        ...
        with open(vendor_obj_list, "r") as f:
            obj_files = f.read().splitlines()
            for obj_file in obj_files:
                obj_file_path = os.path.normpath(
                    os.path.join(self.vendor_path, "t2_os", obj_file)
                )
    ...
    def package(self):
        self.vendor_path = os.path.join(self.clone_path, "platform", "T2")
        ...
        vendor_lib_file = os.path.join(libs_output_path, "libt2Vendor.a")
