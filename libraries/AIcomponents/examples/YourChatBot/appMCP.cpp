...
OPERATE_RET appMCPInit(void)
{
    ...
    // Initialize camera
    if (OPRT_OK !=
        gCamera.begin(CameraResolution::RES_480X480, CAMERA_FPS, CameraFormat::JPEG_YUV422, EncodingQuality::LOW)) {
...
static OPERATE_RET onTakePhoto(MCPPropertyList_t properties, MCPReturnValue_t retVal, void *userData)
{
    ...
    while (1) {
        ...
        PR_DEBUG("YUV422 Camera width: %d, height: %d, dataLen: %d", yuv422Frame.width, yuv422Frame.height,
                 yuv422Frame.dataLen);
    ...
    PR_DEBUG("JPEG Camera width: %d, height: %d, dataLen: %d", jpegFrame.width, jpegFrame.height, jpegFrame.dataLen);
...
static void mcpRegisterExampleTool_2(void)
{
    ...
    TuyaMCPPropDef photoProps[] = {TuyaMCPPropStr("question", "The question prompting the photo capture."),
                                   TuyaMCPPropIntDefRange("count", "Number of photos to capture (1-10).", 1, 1, 10)};
    TUYA_MCP_TOOL_REGISTER("device_camera_take_photo",
                           ...
                           onTakePhoto, nullptr, photoProps, 2);
...
static void mcpRegisterExampleTool_3(void)
{
    ...
    TuyaMCPPropDef volumeProps[] = {TuyaMCPPropIntRange("volume", "The volume level to set (0-100).", 0, 100)};
    TUYA_MCP_TOOL_REGISTER("device_audio_volume_set",
                           "Sets the device's volume level.\n"
                           "Parameters:\n"
                           "- volume (int): The volume level to set (0-100).\n"
                           "Response:\n"
                           "- Returns true if the volume was set successfully.",
                           onSetVolume, nullptr, volumeProps, 1);
...
static void mcpRegisterExampleTool_4(void)
{
    ...
    TUYA_MCP_TOOL_REGISTER("device_audio_mode_set",
